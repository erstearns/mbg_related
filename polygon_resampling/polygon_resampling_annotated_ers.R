                                 ...
                                 raster_col      = NULL)
{
  ...
  # Confirm necessary columns in data ------------------------------------------- needs further review ers------------
  ...
  if(is.null(raster_dict)){
    ...
    d       <- data[, c(unique_vars, 'shapefile', 'location_code', raster_col)]
    pars    <- unique(d) #getting data table of unique combinations of the above fields
    pars    <- pars[!is.na(pars$shapefile), ] #making sure we have a shapefile for each row in unique set
    n_chunk <- nrow(pars) #n_chunk is the number of rows of unique shapefiles and location codes
  }

  ## get row indices
  message('Getting indices and chunking out data by polygon in parallel.\n')
  dx      <- trim(apply(d,   1,paste,collapse='--')) # apply function over rows(1) in d to get one string
  px      <- trim(apply(pars,1,paste,collapse='--'))
  indices <- mclapply(1:n_chunk,function(x){unname(which(dx==px[x]))},mc.cores=cores)

  chunks <- mclapply(1:n_chunk,function(x){data[indices[[x]],]},mc.cores=cores) #This is supposed to be an empty list with same number of elements as there are polygons with columns to match original data input format, ease of rejoining to point data

  # grab population raster - check here that 1k pop raster added
  ...
  if(is.null(raster_dict)){
    ...
    getPointsWrapper.w.dict <- function(x,dens){ #this is the one that should be running ------------------------------------
    ...
    #n.rast <- matrix(unlist(n.rast), ncol = dim(n.rast)[2], nrow = dim(n.rast)[1]) #ers - n.rast not a data frame or matrix from line above, returns a list
    #chunk_points <- apply(n.rast, 1, getPointsWrapper.w.dict) #should be lapply?
    res <- data.table()
    for(r in 1:nrow(n.rast)){
      chunk_points <- rbind(res,data.table(n=n.rast[r,1],getPointsWrapper.w.dict(n.rast[r,],dens=1/10)))
    }
  }

  # duplicate records, and add their integration points --------------------------------------------- what is the point of this?
  ...
  #idx_all <- unlist(indices) #not working bc indices is a list of 12 elements with values of 0 - i don't understand how that was supposed to work
  #data <- data[-idx_all, ]
  #ers - in my case, simply pulling
