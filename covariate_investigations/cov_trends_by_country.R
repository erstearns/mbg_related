...
gaul_list <- get_gaul_codes('africa') # TO DO ------ CHANGE HERE TO COUNTRY/i if loop bfa, cmr, gha---------------------
...
ntv_vars <- c('access','irrigation', 'elevation', 'distrivers', 'growingseason')
tv_vars <- all_cov_layers[!names(all_cov_layers) %in% ntv_vars]

names(tv_vars[[1]]) <- c(2000:2015)
names(tv_vars[[1]]) <- gsub('X', '',names(tv_vars[[1]]))
