...
## Run function to create holdouts (returns list of data.frames with an additional "folds" column)
  #specify n_folds, spat_strat, ss_col, yr_col and temp_strat
  #ss_col - specify number cases per quadtree segment
  table(df$region, df$year)
  long_col = 'longitude'
  lat_col = 'latitude'
  n_folds = as.numeric(n_folds)
  #creating extra column to specify strata for each row of data 
